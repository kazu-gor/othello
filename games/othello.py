...
class Othello:
    ...
    def step(self, action):
        ...
        """ update the board """
        self.board = self.update(self.board)
        """ finished update """
    ...
    def legal_actions(self): # TODO: it need to rewrite to
        legal = []
    ...
    def update(self, board, row, col): # TODO: implement the update function
        """
        Update the Othello board from the action.
        """
        pass

    def is_finished(self): # TODO:it need to rewrite to othello
        if (numpy.sum(self.board==1) == 0 or
            numpy.sum(self.board==-1) == 0 or
            numpy.sum(self.board == 0)):
            return True
        return False
    ...
    def human_input_to_action(self):
        ...
        if (# TODO: 入力値は行列の数字二組が望ましい，example: "12"(row: 1, col: 2)
            len(human_input) == 2
